...
在全连通校验通过的网格上，还可启动**单砖失效审计**：一次低链值遍历找出全部
**关键导向砖**——移除它之后会导致某对入口—避难点断开的砖，并列出每块砖影响的
完整关系集合，帮助改造人员判断网络能否承受任意一块导向砖损坏。

- 技术栈：TypeScript + React 19 + Vite 7
- 单元测试：Vitest（连通路径、行优先挑选规则、JSON 严格校验、单砖失效审计）
- 端到端测试：Playwright（键盘 / 指针编辑、JSON 导入导出、判定主链路、审计交互）
...
连通判定**通过**且入口 / 避难点各 1–4 个时，可点击“开始单砖失效审计”：

- 领域层把入口、导向砖、避难点组成正交无向图，**仅导向砖是可移除顶点**；
  用**一次 DFS 低链值（low-link）遍历**求出全部割点，再按 DFS 子树把端点分组，
  位于不同组的入口—避难点对即该砖移除后断开的关系——不逐砖复制网格、
  不重复执行完整可达搜索。
- **关键砖**按行优先排列；每块砖的受影响关系按入口行优先、再按避难点行优先排列，
  同一关系只出现一次。
- 结果区可选择一块关键砖，画布随即**突出该砖（橙色）及对应端点（青色圈）**。
- 没有任何关键砖时，明确提示“网络可承受任一单砖失效”。
- 任何编辑、合法导入或清空都会**撤销**审计报告；导入被拒绝时保持原网格，
  且不生成审计结果。
...
├── src/
│   ├── components/
│   │   └── GridBoard.tsx     # 12×12 画布（指针 + 键盘编辑、结果与审计高亮）
│   ├── App.tsx               # 工具面板、判定结论、审计报告、导入导出
│   ├── grid.ts               # 核心：类型、正交 BFS、行优先选首不可达对
│   ├── audit.ts              # 单砖失效审计：一次低链值遍历求关键砖与断开关系
│   ├── io.ts                 # JSON 序列化 / 严格解析（整份拒绝）
│   ├── main.tsx
│   └── styles.css
├── tests/
│   ├── unit/                 # Vitest：grid.test.ts、audit.test.ts、io.test.ts
